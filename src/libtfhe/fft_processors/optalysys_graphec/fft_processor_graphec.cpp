...
void FFT_Processor_graphec::execute_reverse_int(cplx* res, const int* a) {
    cplx* out_cplx = (cplx*) out;
    for (int32_t i=0; i<N; i++) in[i]=a[i]/2.;
    for (int32_t i=0; i<N; i++) in[N+i]=-in[i];
    ift_inplace_stable_ns(index_simulator, (double*) in, (double*) out_cplx);
    for (int32_t i=0; i<Ns2; i++) res[i]=out_cplx[2*i+1];
    for (int32_t i=0; i<=Ns2; i++) assert(abs(out_cplx[2*i])<1e-20);
}
...
void FFT_Processor_graphec::execute_direct_Torus32(Torus32* res, const cplx* a) {
    ...
    cplx* in_cplx = (cplx*) in; 
    for (int32_t i=0; i<=Ns2; i++) in_cplx[2*i]=0;
    for (int32_t i=0; i<Ns2; i++) in_cplx[2*i+1]=a[i];
    ft_inplace_stable(index_simulator, (double*) in_cplx, (double*) out);
    for (int32_t i=0; i<N; i++) res[i]=Torus32(int64_t((out[i]*_1sN*_2p32).real()));
